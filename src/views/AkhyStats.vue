...
<template>
  <div v-if="user && !loading" class="w-100">
    ...
    >
      ...
      <v-list-item class="py-4">
        ...
        <v-sparkline
          ...
          color="secondary"
          line-width="1"
          :model-value="elo_graph?.length > 1 ? elo_graph : [0]"
          title="Evolution de l'elo"
        />
      </v-list-item>
    </v-list>
    ...
    >
      ...
      <v-list-item v-for="game in games.filter(g => g.type !== 'POKER_ROUND')" class="pb-4">
        <v-card variant="tonal" color="secondary" rounded="xl">
          <v-card-text class="pb-0">
            <div class="d-flex justify-space-between">
              <div class="d-flex" style="place-items: center">
                ...
                <h3>{{game.p1 === $route.params.id ? users.find(u => u.id ===game.p1)?.globalName : users.find(u => u.id ===game.p2)?.globalName}}</h3>
              </div>
              <div class="d-flex" style="gap: .5rem">
                <h2>{{ game.p1 === $route.params.id ? game.p1_score : game.p2_score }}</h2>
                <h2>-</h2>
                <h2>{{ game.p1 === $route.params.id ? game.p2_score : game.p1_score }}</h2>
              </div>
              <div class="d-flex" style="place-items: center">
                <h3>{{ game.p1 === $route.params.id ? users.find(u => u.id ===game.p2)?.globalName : users.find(u => u.id ===game.p1)?.globalName }}</h3>
